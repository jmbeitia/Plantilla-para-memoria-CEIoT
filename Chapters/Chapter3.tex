...
\begin{itemize}
  \item \code{api/}: la interfaz de programación de aplicaciones (API) y la lógica de pedidos, implementadas con Node.js \cite{nodejs}, Express \cite{express} y TypeScript \cite{typescript} sobre el mapeador objeto-relacional (ORM, del inglés \textit{Object-Relational Mapping}) Prisma \cite{prisma}.
...
La conectividad del sistema comprende tres tramos con características distintas. En la figura \ref{fig:topologia_despliegue} se presenta la topología de despliegue, con los servicios contenerizados del servidor virtual y los componentes instalados en el comercio.
...
Los datos que circulan entre los componentes se serializan en formato \texttt{JSON} (del inglés, \textit{JavaScript Object Notation}). El agente de inteligencia artificial produce una estructura fija, que constituye el contrato interno del sistema. El código \ref{lst:pedido_json} muestra su formato.
...
Las capturas de cada vista del panel se incluyen en el anexo \ref{AppendixA}.
...
Todo el tráfico externo circula cifrado. La pasarela de mensajería entrega los mensajes por HTTPS y el vínculo entre la nube y el borde se resuelve mediante el túnel descripto en la subsección \ref{subsec:tunel}, cuya conexión saliente cifrada evita exponer puertos entrantes en la red del comercio. El servidor de impresión exige, además, un testigo propio en cada petición, de modo que la sola llegada al servicio no habilita la impresión.
