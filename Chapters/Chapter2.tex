...
La vinculación con WhatsApp \cite{whatsapp} se realiza mediante Evolution API \cite{evolutionapi}, una pasarela de mensajería de código abierto que expone las funciones de la plataforma a través de una interfaz REST y de notificaciones automáticas. Evolution API admite dos modos de conexión: uno basado en el protocolo de WhatsApp Web mediante la biblioteca Baileys \cite{baileys}, sin verificación comercial ante Meta, y otro basado en la API oficial WhatsApp Business Cloud \cite{whatsappbusiness}; este trabajo utiliza el primero. Ante cada mensaje entrante, de texto, voz o imagen, la pasarela emite una notificación con el contenido o con las referencias para descargarlo. Junto a Evolution API opera Chatwoot \cite{chatwoot}, una plataforma de gestión de conversaciones que centraliza la bandeja de entrada de WhatsApp y permite al personal consultar el hilo original de cada conversación.

La elección de Evolution API respondió a su costo nulo, su rapidez de puesta en marcha (sin verificación comercial ni aprobación de plantillas) y su despliegue autoalojado, coherente con la infraestructura del sistema. Su limitación es que el modo WhatsApp Web es un mecanismo no oficial, con riesgo de restricciones de estabilidad y cumplimiento; por eso el sistema se diseñó compatible con la API oficial de WhatsApp Business, sin que la migración prevista exija cambios estructurales en los flujos de procesamiento.
...
La transcripción de audio se realiza con el servicio de transcripción de OpenAI, basado en el modelo Whisper \cite{whisper}, elegido por su robustez frente a condiciones adversas de grabación (ruido de fondo, variabilidad de acentos, calidad desigual del micrófono), determinante porque los mensajes de voz se graban dentro del comercio o en la vía pública, sin control sobre el entorno sonoro.

La lectura de las imágenes se resuelve con modelos multimodales de visión, en lugar de un motor OCR clásico, que pierde precisión ante las imágenes recibidas (listas manuscritas con caligrafías dispares e iluminación irregular, y capturas de planillas) por operar sobre cada carácter de manera aislada, sin considerar la disposición del contenido. Los modelos multimodales, en cambio, interpretan la imagen en su contexto, reconocen la estructura tabular y aprovechan el conocimiento del dominio para resolver ambigüedades.

Sobre cada imagen se ejecutan dos modelos de proveedores distintos, GPT-4o de OpenAI \cite{gpt4o} y Claude Sonnet de Anthropic \cite{claudesonnet}, porque ante una misma imagen producen en ocasiones lecturas discrepantes, sobre todo con caligrafías difíciles o planillas complejas; las dos se reconcilian en un tercer paso, donde GPT-5.5 de OpenAI \cite{gpt55} las compara, corrige las diferencias y devuelve una transcripción única.

La estructuración final se resuelve con un modelo extenso de lenguaje (LLM, del inglés \textit{Large Language Model}), GPT-5.5 de OpenAI, integrado como agente dentro del flujo, que recibe el texto del pedido junto con el catálogo del comercio y produce una representación estructurada con los productos, las cantidades y las unidades. Su capacidad de interpretar lenguaje informal, con abreviaturas, errores de tipeo y denominaciones no normalizadas, constituye el aporte central de estos modelos a la solución.

Consumir estos modelos como servicio, en lugar de ejecutarlos sobre la propia infraestructura, define la distribución del cómputo entre el borde y la nube: los tres exigen recursos muy superiores a los del nodo de borde, por lo que su ejecución local no resultaba viable con el equipamiento del comercio, y su consumo como servicio mantiene mínima la inversión inicial y la complejidad operativa. El costo es doble (un componente variable por uso y la dependencia de la conectividad para la interpretación), pero la actuación crítica permanece en el borde, de modo que esa dependencia no compromete la operación local de impresión.
