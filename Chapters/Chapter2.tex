...
El intercambio de mensajes entre los distintos componentes se realiza mediante HTTP (del inglés, \textit{Hypertext Transfer Protocol}), un protocolo de nivel de aplicación basado en peticiones y respuestas, definido en el estándar RFC 9110 \cite{rfc9110}. Las interfaces expuestas siguen el estilo arquitectónico REST (del inglés, \textit{Representational State Transfer}), formulado por Fielding \cite{fielding2000rest}, que organiza la comunicación alrededor de recursos identificados por direcciones y operaciones uniformes sobre ellos.
...
La elección de Evolution API para la etapa inicial del trabajo se fundamentó en tres razones: su costo nulo, la rapidez de puesta en marcha (no requiere el proceso de verificación comercial ni la aprobación de plantillas de la API oficial) y su despliegue autoalojado, coherente con la infraestructura contenerizada del sistema. Su principal limitación reside en que el modo basado en WhatsApp Web constituye un mecanismo no oficial, con posibles restricciones de estabilidad y de cumplimiento frente a la alternativa provista por Meta. Por este motivo, el sistema se diseñó de manera compatible con la API de WhatsApp Business y se encuentra prevista la migración a la vía oficial. Esta compatibilidad se estableció como requerimiento de interoperabilidad desde la planificación del trabajo, de modo que el cambio no implica modificaciones estructurales en los flujos de procesamiento.
...
En esta sección se describen los servicios en la nube sobre los que se apoya el sistema. La plataforma que orquesta los flujos de procesamiento, la infraestructura donde se despliegan los componentes y los servicios de inteligencia artificial que resuelven la interpretación de los pedidos.
...
De manera complementaria se emplea Redis \cite{redis}, una base de datos en memoria de tipo clave-valor, para la gestión de las sesiones y del estado de los clientes activos durante la recepción de un pedido. Redis ofrece tiempos de acceso muy bajos y expiración automática de claves, características apropiadas para datos de carácter transitorio con alta frecuencia de lectura y escritura. La combinación responde a un criterio de diseño explícito: los datos persistentes y auditables residen solo en la base relacional, mientras que Redis se reserva para información temporal. De este modo, una eventual falla de la capa en memoria no compromete los registros del negocio.
